...
var app = angular.module('AngularJSApp', ['ui.bootstrap', 'ngSanitize', 'ngTable', 'ui.select', 'TablePaging', 'ngRoute', 'nvd3', 'ui.ace', 'ngMap']);
...
function BaseController($scope, $uibModal) {
    ...
    $scope.onOpenDialog = function (titulo, mensaje, returnFunction, size, templateUrl, extraData, controller, icono, claseColor, pBackdrop, pKeyboard) {
        ...
        var mBackdrop = (pBackdrop == undefined || pBackdrop == null) ? true : pBackdrop;
        var mKeyboard = (pKeyboard == undefined || pKeyboard == null) ? true : pKeyboard;
...
function CrudController($scope, NgTableParams, ajax, $uibModal, controllerName) {
    ...
    $scope.onDefaultReturnFault = function (data) {
        ...
        if (data.status == 500)
        {
            window.location.href = config.baseURL + '#!/Error500';
        } else {
