...
{
    public class MySQLMapper<T> : ISessionMapper<T> where T : class, IEntity, new()
    {
        public DbSet<T> Session { get; private set; }
        public MySQLService Context { get; private set; }

        public MySQLMapper(DbSet<T> db, MySQLService context)
        {
            Session = db;
            Context = context;
        }
