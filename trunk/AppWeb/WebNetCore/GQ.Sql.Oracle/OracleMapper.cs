...
{
    public class OracleMapper<T> : ISessionMapper<T> where T : class, IEntity, new()
    {
        public DbSet<T> Session { get; private set; }
        public OracleService Context { get; private set; }

        public OracleMapper(DbSet<T> db, OracleService context)
        {
            Session = db;
            Context = context;
        }
